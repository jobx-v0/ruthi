...
export default function SkillsDropdown() {
  ...
  const [isProficiencyDropdownOpen, setIsProficiencyDropdownOpen] =
    useState(false);
  ...
  useEffect(() => {
    ...
    setSkills((prevSkills) =>
      prevSkills.filter((skill) => skill.skill_name && skill.skill_proficiency)
    );
  }, []);
  ...
  useEffect(() => {
    ...
    setAvailableSkills(
      initialSkills.filter((skill) => !selectedSkillNames.includes(skill))
    );
  }, [skills]);
  ...
  const addSkill = () => {
    if (
      (selectedSkill && selectedSkill !== "Other" && selectedProficiency) ||
      ...
      if (
        !skills.some(
          (skill) =>
            skill.skill_name.toLowerCase() === newSkillName.toLowerCase()
        )
      ) {
        const newSkill = {
      ...
      >
          ...
          <div className="flex flex-wrap items-center gap-4">
            ...
            <div className="w-full sm:w-64 relative">
              <button
                type="button"
                onClick={() =>
                  setIsProficiencyDropdownOpen(!isProficiencyDropdownOpen)
                }
        ...
        <div className="bg-white shadow-xl rounded-lg p-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
            {skills
              .filter((skill) => skill.skill_name && skill.skill_proficiency)
              .map((skill, index) => (
                <div
                  key={index}
