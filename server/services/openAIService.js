...
let evaluationByGPTConfig;
try {
  const configPath = path.join(__dirname, "config/dev.config.yaml");
  const config = yaml.load(fs.readFileSync(configPath, "utf8"));
  let evaluationByGPTConfig = config.app.evaluationByGPT;  // Extract evaluationByGPT config
} catch (error) {
  console.error("Error loading YAML config:", error);
  process.exit(1);
}

if(evaluationByGPTConfig){
  ...
  console.log("GPT service is Disabled");
